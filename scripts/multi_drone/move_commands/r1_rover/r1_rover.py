...
class RoverControl(Node):
    def __init__(self):
        ...
        # Subscribe to movement commands (Twist) that will control the robot
        ...
        # Subscribe to laser scanner data for collision avoidance
    ...
    def velocity_callback(self, msg):
        ...
        self.get_logger().info(
            f"Received command: linear velocity {self.linear_velocity}, "
            f"angular velocity {self.angular_velocity}"
        )

    def laser_callback(self, msg):
        """Process laser scanner data to detect obstacles"""
        # Simple obstacle detection example (if distance < 0.5 meters)
        min_distance = np.min(msg.ranges)
        if min_distance < 0.5:
            self.obstacle_detected = True
            self.get_logger().info(f"Obstacle detected at {min_distance:.3f} meters")
        else:
            self.obstacle_detected = False
